...
impl Vec3 {
    ...
    pub fn random_in_unit_disk() -> Self {
        let mut rng = rand::rng();
        loop {
            let p = 2. * Vec3::new(rng.random_range(0.0..1.0), rng.random_range(0.0..1.0), 0.)
                - Vec3::new(1., 1., 0.);
            if p.dot(&p) <= 1. {
                break p;
            }
    ...
    pub fn sqrt(&mut self) {
        self.0[0] = self.0[0].signum() * self.0[0].abs().sqrt();
        self.0[1] = self.0[1].signum() * self.0[1].abs().sqrt();
        self.0[2] = self.0[2].signum() * self.0[2].abs().sqrt();
    }
